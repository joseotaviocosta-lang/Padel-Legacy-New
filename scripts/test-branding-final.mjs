...
// para Windows/Android/iOS/favicon. Hotfix 9.2 também migra o BrandMark
// in-app para exatamente o mesmo raster oficial.
...
check('manifest do ícone não aponta para o master oficial', manifest.master.includes('app-icon-master.png'));
check('manifest ainda declara master alternativo para tamanhos pequenos', !('small' in manifest));
check('gerador derivativo do Hotfix 9.2 ausente', exists('scripts/generate-branding-hotfix-9-2.py'));
...
check('BrandMark.jsx não referencia diretamente o master oficial', brandMark.includes('app-icon-master.png'));
check('BrandMark ainda referencia um SVG alternativo', !brandMark.includes('.svg'));

// ── ASSETS ANTIGOS / PLACEHOLDERS ────────────────────────────────────────────

for (const brandFile of ['index.html', 'src/components/design-system/BrandMark.jsx', 'public/manifest.json']) {
