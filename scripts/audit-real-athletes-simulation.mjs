...
try {
  ...
  // DIAG_TAPER (Fase 7.2/7.3, mantido como instrumento de medição
  // permanente do harness — não é mais opcional para a janela de
  // prioridade, que agora é sempre-ativa em WorldTourLifecycle.js desde
  // a Fase 7.4) — mede reincidência (rank>300 no início da temporada
  // anterior E jogou; ainda rank>300 agora) pra comparar contra a
  // baseline de 90-100% sem a janela e o 0% medido com ela (Fase 7.3
  // §3.3). Puramente de leitura/log — nenhum efeito em gameplay.
