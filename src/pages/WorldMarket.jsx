...
export default function WorldMarket() {
  ...
  return <Page size="default" className="animate-fade-in">
    ...
    {tab === 'athletes' && (athletes.length === 0 ? <EmptyState icon={Users} title="Nenhum atleta encontrado" description="Altere os filtros ou aguarde a próxima atualização mensal." /> : <div className="render-window space-y-2">{athletes.slice(0, visibleCount).map((item) => <div key={item.id} className="glass rounded-2xl p-3 flex items-center gap-3"><div className="w-12 text-center font-black text-primary">#{item.world_rank || '—'}</div><PlayerAvatar name={item.name} size="sm" /><div className="min-w-0 flex-1"><p className="font-bold text-sm truncate">{item.name}</p><p className="flex items-center gap-1 text-[10px] text-muted-foreground"><CountryFlag country={item.country} showLabel /> · OVR {item.overall_rating || 0} · Pot. {item.potential || 0} · {item.market_status}</p></div><div className="hidden sm:block text-right"><p className="font-black text-sm">{money(item.market_value)}</p><p className="text-[9px] text-muted-foreground">valor</p></div><button onClick={() => handleShortlist(item)} className={`p-2 rounded-xl ${reportMap[item.id]?.is_shortlisted ? 'bg-amber-400/15 text-amber-400' : 'bg-secondary/50 text-muted-foreground'}`}><Star className={`h-4 w-4 ${reportMap[item.id]?.is_shortlisted ? 'fill-current' : ''}`} /></button><button onClick={() => openAthlete(item)} className="p-2 rounded-xl bg-primary/15 text-primary"><Eye className="h-4 w-4" /></button></div>)}{visibleCount < athletes.length && <button onClick={() => setVisibleCount((value) => value + PAGE_SIZE)} className="w-full rounded-xl border border-border/60 px-4 py-3 text-sm font-bold text-primary">Carregar mais</button>}</div>)}
    ...
    <ModalShell open={Boolean(selected)} onClose={() => setSelected(null)} title={selected?.data?.name} description={selected ? (selected.type === 'athlete' ? `${selected.data.country || '—'} · #${selected.data.world_rank || '—'}` : `${selected.data.specialty_label} · ${COACH_TIERS[selected.data.tier]?.label || selected.data.tier}`) : null} size="sm">
