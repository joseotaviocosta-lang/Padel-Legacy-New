...
import { Search, RefreshCw, Globe2, Users, Eye, Star, Handshake, Send, UserRoundCog, BriefcaseBusiness, Info, ShieldCheck, Clock, ArrowRightLeft } from 'lucide-react';
import { localGame } from '@/api/localGameClient.js';
import { ensureMyProfile } from '@/lib/padel';
import { PageSkeleton } from '@/components/design-system';
import { Page, PageContent, PageHeader, CardGrid, StatCard, Surface, StatusBadge, EmptyState, ModalShell, PlayerAvatar, CountryFlag, Select } from '@/components/design-system';
...
// Metadados visuais das Movimentações (seção 18): a mesma linguagem de ícone
// já usada em WorldEventCard, aplicada às tags reais do snapshot
// (`recentMovements`), sem inventar categorias novas.
const MOVEMENT_TAG_META = {
  dupla: { icon: Handshake, label: 'Dupla', color: 'text-cyan-400', bg: 'bg-cyan-500/10' },
  treinador: { icon: UserRoundCog, label: 'Treinador', color: 'text-primary', bg: 'bg-primary/10' },
  aposentadoria: { icon: Clock, label: 'Aposentadoria', color: 'text-slate-400', bg: 'bg-slate-500/10' },
  mercado: { icon: ArrowRightLeft, label: 'Mercado', color: 'text-green-400', bg: 'bg-green-500/10' },
};
const MOVEMENT_TAG_PRIORITY = ['dupla', 'treinador', 'aposentadoria', 'mercado'];
function movementMeta(tags) {
  const tag = MOVEMENT_TAG_PRIORITY.find((candidate) => (tags || []).includes(candidate));
  return MOVEMENT_TAG_META[tag] || MOVEMENT_TAG_META.mercado;
}
...
export default function WorldMarket() {
  ...
  return <Page size="default" className="animate-fade-in">
    <PageContent>
      ...
      <Surface padding="compact" className="flex flex-col gap-2 md:flex-row">
        ...
        {tab === 'coaches' && (
          <Select
            value={coachTier}
            onValueChange={setCoachTier}
            aria-label="Categoria de treinador"
            className="sm:w-56"
            options={[{ value: 'all', label: 'Todas as categorias' }, ...Object.entries(COACH_TIERS).map(([key, value]) => ({ value: key, label: value.label }))]}
          />
        )}
      </Surface>

    {tab === 'athletes' && (athletes.length === 0 ? <EmptyState icon={Users} title="Nenhum atleta encontrado" description="Altere os filtros ou aguarde a próxima atualização mensal." /> : <div className="render-window space-y-2">{athletes.slice(0, visibleCount).map((item) => <div key={item.id} className="glass rounded-2xl p-3 flex items-center gap-3"><div className="w-12 text-center font-black text-primary">#{item.world_rank || '—'}</div><PlayerAvatar name={item.name} size="sm" /><div className="min-w-0 flex-1"><p className="font-bold text-sm truncate">{item.name}</p><p className="flex items-center gap-1 text-[10px] text-muted-foreground"><CountryFlag country={item.country} showLabel /> · {item.age ? `${item.age} anos · ` : ''}OVR {item.overall_rating || 0} · Pot. {item.potential || 0} · {item.market_status}</p></div><div className="hidden sm:block text-right"><p className="font-black text-sm">{money(item.market_value)}</p><p className="text-[9px] text-muted-foreground">valor</p></div><button onClick={() => handleShortlist(item)} className={`p-2 rounded-xl ${reportMap[item.id]?.is_shortlisted ? 'bg-amber-400/15 text-amber-400' : 'bg-secondary/50 text-muted-foreground'}`}><Star className={`h-4 w-4 ${reportMap[item.id]?.is_shortlisted ? 'fill-current' : ''}`} /></button><button onClick={() => openAthlete(item)} className="p-2 rounded-xl bg-primary/15 text-primary"><Eye className="h-4 w-4" /></button></div>)}{visibleCount < athletes.length && <button onClick={() => setVisibleCount((value) => value + PAGE_SIZE)} className="w-full rounded-xl border border-border/60 px-4 py-3 text-sm font-bold text-primary">Carregar mais</button>}</div>)}
    ...
    {tab === 'coaches' && <div className="space-y-3"><div className="rounded-2xl border border-primary/20 bg-primary/5 p-3 flex gap-2"><Info className="h-4 w-4 text-primary mt-0.5" /><p className="text-xs text-muted-foreground">O mercado garante opções iniciantes em todos os meses. Técnicos de elite aparecem com menor frequência e exigem carreira mais consolidada.</p></div>{coaches.length === 0 ? <EmptyState icon={UserRoundCog} title="Nenhum treinador disponível" description="Troque a categoria ou aguarde a próxima atualização mensal." /> : <div className="render-window grid md:grid-cols-2 gap-3">{coaches.slice(0, visibleCount).map((coach) => { const tier = COACH_TIERS[coach.tier] || COACH_TIERS.regional; const specialty = COACH_SPECIALTY_INFO[coach.specialty]; return <button key={coach.id || coach.name} onClick={() => setSelected({ type:'coach', data:coach })} className="glass rounded-2xl p-4 text-left hover:border-primary/40 border border-transparent transition"><div className="flex items-start gap-3"><PlayerAvatar name={coach.name} /><div className="flex-1 min-w-0"><div className="flex items-center gap-2"><p className="font-black text-sm truncate">{coach.name}</p><span className={`text-[9px] px-2 py-0.5 rounded-full ${tier.bg} ${tier.color}`}>{tier.label}</span></div><p className="text-[10px] text-muted-foreground">{specialty?.label || coach.specialty_label} · Rep. {coach.reputation || 0}</p><p className="mt-2 text-xs text-muted-foreground line-clamp-2">{specialty?.summary || coach.philosophy}</p><div className="mt-3 flex justify-between"><span className="text-xs font-black text-primary">{money(coach.market_salary)}/mês</span><span className="text-[10px] text-muted-foreground">Demanda {String(coach.market_demand || 'media').replace('_',' ')}</span></div></div></div></button>; })}{visibleCount < coaches.length && <button onClick={() => setVisibleCount((value) => value + PAGE_SIZE)} className="md:col-span-2 w-full rounded-xl border border-border/60 px-4 py-3 text-sm font-bold text-primary">Carregar mais</button>}</div>}</div>}

    {tab === 'movements' && ((snapshot?.recentMovements || []).length === 0 ? <EmptyState icon={BriefcaseBusiness} title="Sem movimentações recentes" description="Avance o calendário para acompanhar trocas de dupla, aposentadorias e mudanças de mercado." /> : <div className="render-window space-y-2">{snapshot.recentMovements.slice(0, visibleCount).map((event) => { const meta = movementMeta(event.tags); const MoveIcon = meta.icon; return <div key={event.id} className="glass rounded-2xl p-4 flex gap-3"><div className={`h-9 w-9 shrink-0 rounded-lg ${meta.bg} flex items-center justify-center`}><MoveIcon className={`h-4 w-4 ${meta.color}`} /></div><div className="min-w-0 flex-1"><div className="flex justify-between gap-3"><p className="font-bold text-sm">{event.title}</p><span className="shrink-0 text-[10px] text-muted-foreground whitespace-nowrap">{event.event_date}</span></div><p className="mt-1 text-xs text-muted-foreground">{event.content}</p></div></div>; })}{visibleCount < snapshot.recentMovements.length && <button onClick={() => setVisibleCount((value) => value + PAGE_SIZE)} className="w-full rounded-xl border border-border/60 px-4 py-3 text-sm font-bold text-primary">Carregar mais</button>}</div>)}

    <ModalShell open={Boolean(selected)} onClose={() => setSelected(null)} title={selected?.data?.name} description={selected ? (selected.type === 'athlete' ? `${selected.data.country || '—'}${selected.data.age ? ` · ${selected.data.age} anos` : ''} · #${selected.data.world_rank || '—'}` : `${selected.data.specialty_label} · ${COACH_TIERS[selected.data.tier]?.label || selected.data.tier}`) : null} size="sm">
      {selected && <div className="space-y-4"><PlayerAvatar name={selected.data.name} size="lg" />
