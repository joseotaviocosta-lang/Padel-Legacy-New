...
import { Search, RefreshCw, Globe2, Users, Eye, Star, Handshake, Send, UserRoundCog, BriefcaseBusiness, Info, ShieldCheck } from 'lucide-react';
import { localGame } from '@/api/localGameClient.js';
import { ensureMyProfile } from '@/lib/padel';
import { PageSkeleton } from '@/components/design-system';
import { Page, PageContent, PageHeader, CardGrid, StatCard, Surface, StatusBadge, EmptyState, ModalShell } from '@/components/design-system';
...
export default function WorldMarket() {
  ...
  return <Page size="default" className="animate-fade-in">
    ...
    <ModalShell open={Boolean(selected)} onClose={() => setSelected(null)} title={selected?.data?.name} description={selected ? (selected.type === 'athlete' ? `${selected.data.country || '—'} · #${selected.data.world_rank || '—'}` : `${selected.data.specialty_label} · ${COACH_TIERS[selected.data.tier]?.label || selected.data.tier}`) : null} size="sm">
      {selected && <div className="space-y-4"><div className="h-12 w-12 rounded-2xl bg-primary/15 text-primary flex items-center justify-center font-black">{initials(selected.data.name)}</div>
