...
export default function WorldMarket() {
  ...
  return <div className="px-4 md:px-8 py-5 max-w-6xl mx-auto space-y-4 animate-fade-in">
    ...
    {tab === 'movements' && ((snapshot?.recentMovements || []).length === 0 ? <EmptyStateCard icon={BriefcaseBusiness} title="Sem movimentações recentes" message="Avance o calendário para acompanhar trocas de dupla, aposentadorias e mudanças de mercado." /> : <div className="render-window space-y-2">{snapshot.recentMovements.slice(0, visibleCount).map((event) => <div key={event.id} className="glass rounded-2xl p-4"><div className="flex justify-between gap-3"><p className="font-bold text-sm">{event.title}</p><span className="text-[10px] text-muted-foreground whitespace-nowrap">{event.event_date}</span></div><p className="mt-1 text-xs text-muted-foreground">{event.content}</p></div>)}{visibleCount < snapshot.recentMovements.length && <button onClick={() => setVisibleCount((value) => value + PAGE_SIZE)} className="w-full rounded-xl border border-border/60 px-4 py-3 text-sm font-bold text-primary">Carregar mais</button>}</div>)}
