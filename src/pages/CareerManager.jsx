...
export default function CareerManager() {
  ...
  const create = (event) => { event?.preventDefault(); return run(async () => {
    setCreating(true);
    try {
  ...
  return <div className="min-h-screen bg-background text-foreground p-4 md:p-8"><div className="max-w-5xl mx-auto">
    ...
    {showCreate && <div className="fixed inset-0 z-50 bg-black/70 p-4 flex items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="new-career-title"><form onSubmit={create} className="glass w-full max-w-md rounded-2xl p-6 border border-primary/30"><h2 id="new-career-title" className="text-2xl font-black">Identifique seu save</h2><p className="text-sm text-muted-foreground mt-1">O nome do atleta continuará sendo definido pelo tutorial.</p><label htmlFor="save-name" className="block text-sm font-semibold mt-5 mb-2">Nome da carreira</label><input id="save-name" autoFocus value={saveName} onChange={(e) => setSaveName(e.target.value)} maxLength={40} placeholder="Ex.: Minha carreira principal" className="w-full rounded-xl border border-border bg-background px-4 py-3 outline-none focus:border-primary"/><div className="mt-1 text-right text-xs text-muted-foreground">{saveName.length}/40</div><div className="flex justify-end gap-2 mt-5"><button type="button" onClick={() => { setShowCreate(false); setSaveName(''); setError(''); }} className="rounded-lg border px-4 py-2 text-sm">Cancelar</button><button type="submit" disabled={creating || !saveName.trim()} className="rounded-lg bg-primary text-primary-foreground px-4 py-2 text-sm font-bold disabled:opacity-50">{creating ? 'Criando...' : 'Criar carreira'}</button></div></form></div>}
  </div></div>;
}
