...
export async function finalizeSeason({ profile, partner, force = false }) {
  ...
  await Promise.allSettled([
    ...
    localGame.entities.CareerMessage.create({ profile_id: profile.id, sender_name: 'Circuito Padel Legacy', sender_type: 'federacao', title: `Relatório final de ${year}`, subject: `Relatório final de ${year}`, content: `${summary} Bônus: ${totals.coins} moedas, ${totals.xp} XP e ${totals.rankPoints} pontos de ranking.`, body: `${summary} Bônus: ${totals.coins} moedas, ${totals.xp} XP e ${totals.rankPoints} pontos de ranking.`, status: 'nao_lida', message_type: 'season_report', notification_type: 'SEASON_REPORT' }),
