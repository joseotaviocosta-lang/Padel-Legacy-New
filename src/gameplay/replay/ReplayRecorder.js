...
export function appendPointToReplay(replay, before, after, result) {
  ...
  shots.forEach((entry, index) => {
    ...
    recorder.record({ type: 'shot', actor_id: actor.id, point_id: pointId, rally_id: rallyId, data: { shot_type: entry.shot, hand: entry.shot === 'backhand' ? 'backhand' : 'forehand', origin: position, target, ball_speed_kmh: entry.shot === 'smash' ? 118 : 72, spin: entry.shot === 'lob' ? 'lob' : 'flat', outcome: index === shots.length - 1 ? (result.outcome || result.result) : 'in_play' } });
  ...
  recorder.record({ type: result.outcome === 'WINNER' || result.result === 'winner' ? 'winner' : 'error', actor_id: result.errorPlayer?.id || result.finisher?.id, point_id: pointId, rally_id: rallyId, data: { winning_team: result.winnerTeamId, winner_player_id: result.winnerPlayer?.id || null, error_player_id: result.errorPlayer?.id || null, serving_team: result.servingTeamId, server_player_id: result.serverPlayerId, shot_type: result.shot, error_shot: result.errorShot || null, outcome: result.outcome || result.result, forced: Boolean(result.forcedError), winner_position: result.winnerPosition, point_ending_context: result.pointEndingContext } });
  recorder.record({ type: 'point_end', point_id: pointId, rally_id: rallyId, data: { winner: result.winnerTeamId, loser: result.loserTeamId, serving_team: result.servingTeamId, server_player_id: result.serverPlayerId, outcome: result.outcome || result.result } });
