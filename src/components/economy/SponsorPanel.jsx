...
import { getMonthlySponsorMarket } from '@/lib/sportsEconomyV26';
...
export default function SponsorPanel({ profile, contracts, onSign, onTerminate, onRenew, busy }) {
  ...
  const sponsorMarket = useMemo(() => getMonthlySponsorMarket(profile, SPONSOR_CATALOG), [profile]);
  const allSponsors = useMemo(() => (SPONSOR_CATALOG || []).filter(s => s && s.id && s.name), []);
  const validSponsors = useMemo(() => (sponsorMarket.sponsors || []).filter(s => s && s.id && s.name), [sponsorMarket]);
  ...
  return (
    <div className="space-y-4">
      ...
      <GlassCard><h2 className="font-bold text-sm mb-2">Slots de patrocínio</h2><div className="grid grid-cols-2 md:grid-cols-5 gap-2">{Object.entries(SPONSOR_SLOT_LIMITS).map(([category, limit]) => { const used = activeContracts.filter((contract) => { const sponsor = allSponsors.find((item) => item.id === contract.sponsor_id || item.name === contract.sponsor_name); return (contract.sponsor_category || (sponsor && getSponsorCategory(sponsor))) === category; }).length; return <div key={category} className="rounded-lg bg-secondary/30 p-2"><p className="text-[9px] uppercase text-muted-foreground">{category}</p><p className="text-sm font-black">{used}/{limit}</p></div>; })}</div><p className="text-[10px] text-muted-foreground mt-2">Marcas da mesma categoria competem pelo mesmo espaço. Contratos legados são preservados.</p></GlassCard>
      <GlassCard>
        <div className="flex flex-col sm:flex-row sm:items-center gap-2">
          <div className="flex-1">
            <p className="text-sm font-bold">Mercado de patrocínios · {sponsorMarket.month}</p>
            <p className="text-[10px] text-muted-foreground">Ofertas compatíveis com a fase {sponsorMarket.stage}. O mercado é renovado a cada mês da carreira.</p>
          </div>
          <span className="px-2 py-1 rounded-lg bg-primary/10 text-primary text-[10px] font-bold">{validSponsors.length} marcas disponíveis</span>
        </div>
      </GlassCard>
      {/* Active contracts */}
      <GlassCard>
        ...
        {activeContracts.length === 0 ? (
          ...
          <div className="space-y-2">
            {activeContracts.map((c, i) => {
              const sponsor = allSponsors.find(s => s?.id === c?.sponsor_id)
                || allSponsors.find(s => String(s?.name || '').toLowerCase() === String(c?.sponsor_name || '').toLowerCase());
