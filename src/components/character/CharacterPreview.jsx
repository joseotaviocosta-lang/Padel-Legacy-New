...
import { SKIN_TONES, HAIR_COLORS, FLAGS, VOICE_TYPES, CELEBRATIONS } from '@/lib/characterCatalog';
...
export default function CharacterPreview({ data, profile }) {
  ...
  return (
    <div className="glass rounded-2xl p-5 sticky top-20">
      ...
      {/* Avatar */}
      <div className={`flex flex-col items-center origin-top transition-transform ${heightScale}`}>
        {/* Hat layer */}
        ...
        {/* Head */}
        <div className="relative w-16 h-16 rounded-full" style={{ background: skin }}>
          {/* Hair */}
          {data.hair_style !== 'rapado' && (
            <div className={`absolute -top-1 -inset-x-1 h-5 rounded-t-full ${data.hair_style === 'longo' ? 'h-8' : ''}`} style={{ background: hair }} />
          )}
          ...
          {/* Eyes */}
          <div className="absolute top-7 left-4 w-1.5 h-1.5 rounded-full bg-slate-800" />
          <div className="absolute top-7 right-4 w-1.5 h-1.5 rounded-full bg-slate-800" />
          {/* Glasses */}
