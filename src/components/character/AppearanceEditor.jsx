...
import { ColorPicker, OptionGrid, SliderRow, LockedControl } from './CharacterShared';
...
export default function AppearanceEditor({ data, update }) {
  return (
    <div className="space-y-5">
      ...
      <LockedControl locked={isFieldLocked(data, 'build')}>
        <OptionGrid label="Biotype" options={BUILDS} value={data.build} onChange={v => update('build', v)} />
      </LockedControl>
      <LockedControl locked={isFieldLocked(data, 'height_cm')}>
