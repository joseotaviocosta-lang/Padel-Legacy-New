...
export function getCareerRecommendations(profile, context = {}) {
  ...
  else if (Number(profile.energy || 0) < 30) add({ id: 'recover-energy', priority: 95, importance: 'recomendada', title: 'Recupere energia', explanation: 'Competir cansado reduz desempenho e aumenta o risco de lesão.', route: APP_ROUTES.TRAINING_AGENDA, actionLabel: 'Planejar dia livre' });
  if (profile.play_style && profile.recommended_training_attributes?.length && !(context.trainings || []).length) {
    ...
    add({ id: 'profile-training', priority: 90, importance: 'recomendada', title: `Treine como ${profile.archetype_label || 'seu arquétipo'}`, explanation: priorities.length ? `Priorize ${priorities.join(', ')}.` : 'Escolha um treino alinhado ao seu perfil.', route: APP_ROUTES.TRAINING, actionLabel: 'Ver treinos' });
  }
  ...
  if (!recommendations.length) add({ id: 'plan-week', priority: 40, importance: 'opcional', title: 'Planeje a próxima semana', explanation: 'Alterne treino, dias livres e competição para evoluir com consistência.', route: '/game/calendar', actionLabel: 'Abrir calendário' });
